...
object ModuleGroupProcessor {
    ...
    fun parseModulesGroup(modulesElements: Set<Element>, logger: Logger, mFiler: Filer, elements: Elements): JsonArray? {
        if (CollectionUtils.isNotEmpty(modulesElements)) {
            ...
            for (element in modulesElements) {
                val group = element.getAnnotation<ModuleGroup>(ModuleGroup::class.java)
                val units = group.value
    ...
    @Throws(IOException::class)
    fun parseModules(modulesElements: Array<ModuleUnit>, element: Element, logger: Logger, mFiler: Filer, elements: Elements): JsonArray? {
        if (modulesElements.size > 0) {
